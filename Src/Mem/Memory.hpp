...
protected:
	...
	virtual ~GMemory();
private:
	STATIC VOID MapMark(UINT bit);STATIC
	VOID MapUnmark(UINT bit);STATIC
	BOOL MapTest(UINT bit);STATIC
	INT MapFirstFree();

public:
protected:
	STATIC
	UINT *m_pu_memMapStart, *m_pu_memMapEnd;STATIC
	UINT m_ui_memMapLen;STATIC
	UINT m_ui_memTotal;STATIC
	UINT m_ui_memAvail;STATIC
	UINT m_ui_memMaxBlks;STATIC
	UINT m_ui_memUsedBlks;STATIC
	UINT m_ui_memFreeBlks;
private:
};
