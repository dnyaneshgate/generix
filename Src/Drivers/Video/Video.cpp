...
INT GVideo::Write(const CHAR *buffer) {
	return m_textMode.WriteString(buffer);
}

VOID GVideo::Put(const CHAR ch) {
...
VOID GVideo::SetFontColor(Console::Color fColor) {
	m_textMode.SetFontColor(fColor);
}

VOID GVideo::SetBackColor(Console::Color bColor) {
	m_textMode.SetBackColor(bColor);
}

Console::Color GVideo::GetFontColor() {
	return m_textMode.m_FontColor;
}

Console::Color GVideo::GetBackColor() {
	return m_textMode.m_BackColor;
}

VOID GVideo::SetX(UINT X) {
	m_textMode.m_u_X = X;
}

VOID GVideo::SetY(UINT Y) {
	m_textMode.m_u_Y = Y;
}
...
UINT GVideo::GetWidth() {
	return m_textMode.m_u_Width;
}

UINT GVideo::GetHeight() {
